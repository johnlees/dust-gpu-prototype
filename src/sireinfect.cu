...
int main (int argc, char **argv) {
  ...
  while ((c = getopt (argc, argv, "abgSIps:")) != -1)
    switch (c) {
      ...
      case 'I':
        ...
        break;
      case 'p':
        n_particles_in = optarg;
        n_particles = atoi(n_particles_in);
        break;
      case 's':
        n_steps_in = optarg;
  ...
  internal.initial_R = 0;
  internal.I_ini = I_ini;
  internal.alpha = alpha;
  internal.beta = beta;
  internal.gamma = gamma;
  internal.S_ini = S_ini;
  internal.initial_I = internal.I_ini;
  internal.initial_S = internal.S_ini;
  internal.p_IR = 1 - std::exp(- internal.gamma);
  internal.p_RS = 1 - std::exp(- internal.alpha);

  dust::Dust<sireinfect> dust_obj(data, 0, {0}, n_particles, 1);
...
// R interface - not trying to compile this yet
/*
#include <array>
#include <Rcpp.h>
