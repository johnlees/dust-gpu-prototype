...
inline void Xoshiro<T>::set_seed(uint64_t seed) {
  ...
  cudaMemcpyFromSymbol(_state, state, XOSHIRO_WIDTH * sizeof(uint64_t), 0, cudaMemcpyDefault)
}
...
inline void Xoshiro<T>::jump() {
  ...
  uint64_t state[XOSHIRO_WIDTH];
  cudaMemcpyFromSymbol(state, _state, XOSHIRO_WIDTH * sizeof(uint64_t), 0, cudaMemcpyDefault);
  uint64_t s0 = 0;
  uint64_t s1 = 0;
  uint64_t s2 = 0;
  uint64_t s3 = 0;
